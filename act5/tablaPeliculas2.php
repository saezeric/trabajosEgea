...
function generate_ratings($rating) {
    $movie_rating = '';
    for ($i = 0; $i < $rating; $i++) {
        $movie_rating .= '<img src="https://i.pinimg.com/originals/e8/91/66/e891669c27c833ff0f2db2c083344117.png" alt="star" width="20px" />';
    }
    return $movie_rating;
}
...
function avg_rating($avg_rating) {

    $query = "SELECT AVG(review_rating) AS avgRating FROM reviews";
    $result = mysqli_query($db, $query) or die(mysqli_error($db));
    $row = mysql_fetch_assoc($result);
    $avgRating = $row['avgRating'];

echo "<h2>Review Average: </h2>";
